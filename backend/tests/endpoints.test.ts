...
describe('HTTP Endpoints - Articles', () => {
    ...
    it('POST /articles should create an article (Author Only)', async () => {
        ...
        (prisma.article.create as any).mockResolvedValue({ ...payload, id: 'art-1', authorId: 'author-id' });
    ...
    it('GET /articles/:id should return 404 if deleted', async () => {
        (prisma.article.findUnique as any).mockResolvedValue({ id: '1', deletedAt: new Date() });
    ...
    it('DELETE /articles/:id should return 403 if not the author', async () => {
        (prisma.article.findUnique as any).mockResolvedValue({ id: '1', authorId: 'someone-else' });
    ...
    it('PUT /articles/:id should update an article', async () => {
        (prisma.article.findUnique as any).mockResolvedValue({ id: '1', authorId: 'author-id' });
        (prisma.article.update as any).mockResolvedValue({ id: '1', title: 'Updated' });
    ...
    it('GET /articles/me should return author content including drafts', async () => {
        (prisma.article.findMany as any).mockResolvedValue([{ id: '1', status: 'Draft' }]);
        (prisma.article.count as any).mockResolvedValue(1);
...
describe('HTTP Endpoints - Auth', () => {
    it('POST /auth/signup should return 201 on success', async () => {
        ...
        (prisma.user.create as any).mockResolvedValue({ ...payload, id: 'user-1' });
